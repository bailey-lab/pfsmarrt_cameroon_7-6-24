...
This version uses facets instead of subplots - I eventually abandoned it because
the 'jitter' value couldn't be set wide enough for points to appear as anything
other than a line (I think because there's an invisible 'box' plot to the side
of each of the strip plots). Getting averages and standard deviations to plot
correctly was also a struggle.
'''
...
fig=px.strip(df, x='concentration', y='log2_read_count', hover_data=['samples'], facet_col='amplicon', facet_col_wrap=4, width=4000, height=6000).update_traces(jitter = 1)
#	fig.layout.update(go.Layout(barmode = 'overlay',))
