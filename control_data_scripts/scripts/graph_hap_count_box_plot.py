...
COI_path='/nfs/jbailey5/baileyweb/asimkin/other_people/jmsadler/bigv10ctrl/analyzed_by_PCR_replicates/seekdeep_analysis_snakemake/half_percent_exact_only_analysis_graphs/samp_hap_counts.tsv'
...
subprocess.call(['mkdir', 'hap_count_box_plots'])
for amplicon in conc_dict:
	data={"concentration": concentrations, "COI": conc_dict[amplicon], "samples": samples}
	df = pd.DataFrame(data)
	fig=px.box(df, x="concentration", y="COI", points="all", hover_data=['samples'])
	fig['layout']['title']=amplicon
#	print(fig)
#	print(amplicon)
#	fig.show()
	fig.write_html(f'hap_count_box_plots/{amplicon}_box_plot.html')
#	exit()